...
target = """        if (intent?.action == "BIOMETRIC_SUCCESS") {
            smartAutomationManager.handleManualDismiss()
            showFloatingBubbleInternal()
            return START_STICKY
        }"""

replacement = """        if (intent?.action == "BIOMETRIC_SUCCESS" || intent?.action == "UNLOCK_SCREEN") {
            smartAutomationManager.handleManualDismiss()
            showFloatingBubbleInternal()
            return START_STICKY
        }"""
...
if target in content:
    ...
    with open('app/src/main/java/com/noxscreen/app/BlackScreenService.kt', 'w') as f:
        f.write(content)
    print("Replaced Service intent!")
else:
    print("Service intent not found")
